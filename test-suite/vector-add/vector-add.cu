...
__global__
void vecGPU(float *a, float *b, float *c, int n, int p);
...
int main(int argc, char *argv[]){
	...
	/* TODO: 	
		...
		*/
		int num_blocks = 8;
		int num_threads = 500;
		int p = ceil(n/float(num_threads * num_blocks));
		vecGPU<<<num_blocks, num_threads>>>(ad, bd, cd, n, p);
